(function () {
    ...
    const promptTemplate = (cwd="~") =>
        `<span class="prompt">user@user</span>:<span class="command">${cwd}</span>$ `;
    ...
    function handleCommand(cmd) {
        ...
        switch (trimmed) {
            case "pwd":
                printLine("/home/user" + (currentDir === "~" ? "" : currentDir.replace("~", "")));
                break;
            ...
            case "saados":
                printLine("Windows18 is based on SaadOS Web Edition — built by Saad.");
                break;
